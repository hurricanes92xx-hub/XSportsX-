...
function networkFor(meta) {
  ...
  const rules = [
    ["ESPN2", ["espn2"]], ["ESPNU", ["espnu"]], ["ESPN", ["espn"]], ["NFL Network", ["nfl network"]], ["NHL Network", ["nhl network"]],
    ["MLB Network", ["mlb network"]], ["NBA TV", ["nba tv"]], ["ACC Network", ["acc network", "accn"]], ["Big Ten Network", ["big ten network", "btn"]],
    ["SEC Network", ["sec network", "secn"]], ["Big 12 Now", ["big 12 now", "big12 now"]], ["FOX Sports 1", ["fox sports 1", "fs1"]],
    ["FOX Sports 2", ["fox sports 2", "fs2"]], ["CBS Sports Network", ["cbs sports network", "cbssn"]], ["ABC", ["abc"]],
...
export function buildM3U(metas, baseUrl) {
  ...
  for (const meta of metas || []) {
    ...
    const id = String(meta.id), network = networkFor(meta), logo = meta.logo || meta.poster || meta.background || "";
    const name = String(meta.name).replace(/\s+/g, " ").trim(), group = `Sports / ${network}`;
...
export function buildXMLTV(metas) {
  ...
  for (const meta of metas || []) {
    if (!meta?.id || !meta?.name) continue;
    const id = String(meta.id), network = networkFor(meta), logo = meta.logo || meta.poster || meta.background || "";
    const start = startTime(meta), stop = new Date(start.getTime() + 3 * 60 * 60 * 1000);
