...
export function matchScore(channel, event) {
  const hay=normalizeName(`${channel.name} ${channel.group} ${channel.id}`), home=normalizeName(`${event?.home?.name} ${event?.home?.short}`), away=normalizeName(`${event?.away?.name} ${event?.away?.short}`), league=normalizeName(event?.league||"");
  let score=0; if(home&&hay.includes(home))score+=30; if(away&&hay.includes(away))score+=30; if(event?.home?.short&&hay.includes(normalizeName(event.home.short)))score+=20; if(event?.away?.short&&hay.includes(normalizeName(event.away.short)))score+=20; if(league&&hay.includes(league))score+=10; score+=Math.round(similarity(channel.name,event.title)*20);
  const channelTime=channel.start||channel.date||channel.datetime;
  if(channelTime&&event?.start){ const delta=Math.abs(new Date(channelTime).getTime()-new Date(event.start).getTime()); if(Number.isFinite(delta)){if(delta<=15*60_000)score+=20; else if(delta<=60*60_000)score+=10;} }
...
export async function fetchWithTimeout(url,options={},timeoutMs=9000){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeoutMs);try{return await fetch(url,{redirect:"follow",...options,signal:controller.signal,headers:{"User-Agent":"XSportsX/3.0 Nuvio addon",...(options.headers||{})}});}finally{clearTimeout(timer);}}
