...
function filterPayload(body, token, path) {
  ...
  if (path === '/manifest.json') {
    ...
    if (Array.isArray(body.catalogs)) body.catalogs = body.catalogs.filter(c =>
      c.id === 'sports-command-center' || c.id === 'live-now' || c.id === 'starting-soon' || c.id === 'iptv-live' || selected.has(c.id)
    );
  }
  if (Array.isArray(body.metas)) body.metas = body.metas.filter(meta => {
...
const proxy = http.createServer((req, res) => {
  try {
    ...
    const upstream = http.request({hostname:'127.0.0.1',port:internalPort,path,method:req.method,headers:{...req.headers,host:`127.0.0.1:${internalPort}`}}, response => {
      ...
      if (!filter) { res.writeHead(response.statusCode || 502, response.headers); return response.pipe(res); }
      const chunks = [];
      ...
      response.on('end', () => {
        try {
          ...
          const h = {...response.headers,'content-length':String(out.length),'cache-control':'no-store, no-cache, must-revalidate'};
