...
const PUBLIC_SEGMENTS=new Set(['manifest.json','configure','health','xtream-health','artwork','catalog','meta','stream','qr']);
const PRIVATE_RESOURCES=new Set(['manifest.json','catalog','meta','stream']);
function preparePrivateRequest(req){
  let raw=String(req.url||'/');
  if(raw.startsWith('/v527/')){raw=raw.slice(5)||'/';req.url=raw;}
  const u=new URL(raw,'http://local');
  const parts=u.pathname.split('/').filter(Boolean);
  const token=parts[0];
  if(token&&!PUBLIC_SEGMENTS.has(token)&&parts.length>=2&&PRIVATE_RESOURCES.has(parts[1])){
    const cfg=decryptConfig(token);
    if(cfg){
      const rest='/' + parts.slice(1).join('/');
      req.url=rest+(u.search||'');
      req._xsportsxConfig=cfg;
      return cfg;
    }
  }
  return null;
...
function requestConfig(req){
  const direct=preparePrivateRequest(req);
  if(direct)return normalize(direct);
  return normalize(decryptConfig(req.query?.config)||null);
}
app.use((req,res,next)=>{ctx.run(requestConfig(req),next);});
...
app.get('/artwork/:league.svg',(req,res)=>res.type('image/svg+xml').send(artworkSvg(req.params.league)));
app.get('/artwork/event/:league/:id.svg',async(req,res)=>{const e=await eventById(req.params.league,req.params.id);if(!e)return res.type('image/svg+xml').send(artworkSvg(req.params.league));res.type('image/svg+xml').send(eventArtworkSvg(e.league,e.away.name,e.home.name,e.state,e.start,e.away.logo,e.home.logo,e.away.short,e.home.short));});
app.get('/qr',async(req,res)=>{try{const data=String(req.query.data||'');if(!data)return res.status(400).end();res.type('png').send(await QRCode.toBuffer(data,{width:420,margin:2,dark:'#050811',light:'#ffffff'}));}catch(e){res.status(400).end();}});

function setupHtml(req){const b=baseUrl(req);return`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>XSportsX Setup</title></head><body><main><h1>XSportsX</h1><p>Connect Xtream or M3U, match your channels to live events, and use the same private setup on every device.</p><input id="base"><input id="user"><input id="pass" type="password"><button onclick="generate()">CREATE PRIVATE SPORTS CONNECTION</button><div id="result"></div><script>async function generate(){const body={source:'xtream',sports:Object.keys(${JSON.stringify(LEAGUES)}) ,xtream:{baseUrl:base.value,username:user.value,password:pass.value}};const r=await fetch('/configure',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}),d=await r.json();document.getElementById('result').innerHTML=r.ok?'<a href="'+d.manifestUrl+'">'+d.manifestUrl+'</a>':'<p>'+d.error+'</p>'}</script></main></body></html>`;}
