...
function classify(m) {
  const t = `${m.name || ""} ${m.description || ""} ${(m.genres || []).join(" ")}`.toLowerCase();
  return { t, live: /(^|\W)live(\W|$)|🔴/.test(t), soon: /starting soon|⏰/.test(t) };
}
...
async function loadUpstream() {
  ...
  for (const [id, name, sport, espnLeague] of leagueConfigs) {
    const candidates = [`http://127.0.0.1:${UPSTREAM_PORT}/catalog/sport/${id}.json`, `http://127.0.0.1:${UPSTREAM_PORT}/catalog/sport/upcoming.json`];
    let data = null;
    for (const u of candidates) { try { data = await getJson(u); if (Array.isArray(data?.metas) && data.metas.length) break; } catch {} }
    if (Array.isArray(data?.metas)) out.push(...data.metas.map(m => normalizeMeta(m, id)));
...
function filtered(metas, mode) {
  ...
  if (["nfl","ncaaf","nba","nhl","mlb","soccer"].includes(mode)) return metas.filter(m => classify(m).t.includes(mode) || (mode === "ncaaf" && /college|ncaa|cfb|football/.test(classify(m).t)) || (mode === "soccer" && /soccer|premier|mls|la liga/.test(classify(m).t)));
...
function manifest() {
  ...
  return { id: ADDON_ID, version: VERSION, name: "XSportsX Sports Command Center", description: "Ultimate sports command center with live-first discovery, large landscape artwork, ESPN-powered event fallback, UFC, NCAA Football, networks, EPG and stream resolution.", logo: "https://a.espncdn.com/i/teamlogos/leagues/500/nfl.png", background: "https://a.espncdn.com/i/teamlogos/leagues/500/nfl.png", resources: [{ name: "catalog", types: ["channel"] }, { name: "meta", types: ["channel"], idPrefixes: ["sport:"] }, { name: "stream", types: ["channel"], idPrefixes: ["sport:"] }], types: ["channel"], idPrefixes: ["sport:"], behaviorHints: { configurable: false, configurationRequired: false }, catalogs: catalogs.map(([id, name]) => ({ type: "channel", id, name, showInHome: true })), liveTv: { enabled: true, name: "XSportsX Sports Live TV", playlist: `${BASE}${PREFIX}/live-tv.m3u`, epg: `${BASE}${PREFIX}/epg.xml`, guide: `${BASE}${PREFIX}/catalog/channel/sports-command-center.json`, refreshSeconds: 60 } };
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (path === "/health") return json(res, { ok: true, version: VERSION, addonId: ADDON_ID, commandCenter: true, catalogFallback: true });
