...
function json(res, body, status = 200) { res.writeHead(status, { "content-type": "application/json; charset=utf-8", "cache-control": "no-store, no-cache, max-age=0, must-revalidate", pragma: "no-cache", expires: "0", "access-control-allow-origin": "*", "x-xsportsx-version": VERSION, "x-xsportsx-addon-id": ADDON_ID }); res.end(JSON.stringify(body)); }
...
function normalizeMeta(m, league) { const id = String(m.id || `sport:${league}-${Math.random().toString(36).slice(2)}`); return { ...m, id: id.startsWith("sport:") ? id : `sport:${id}`, type: "channel", posterShape: "landscape", __xsportsLeague: league, genres: [...new Set([...(m.genres || []), "Sports", league.toUpperCase()])], behaviorHints: { ...(m.behaviorHints || {}) } }; }
    ...
    try {
      ...
      if (Array.isArray(data?.metas)) {
        for (const m of data.metas) {
          const declared = text(m);
          const conflicts = (id === "nfl" && /college football|ncaa|cfb|ncaaf/.test(declared)) ||
            (id === "ncaaf" && /\bnfl\b|national football league/.test(declared)) ||
            (id === "nba" && /college basketball|ncaa/.test(declared)) ||
            (id === "nhl" && /college hockey|ncaa/.test(declared)) ||
            (id === "mlb" && /college baseball|ncaa/.test(declared));
          if (!conflicts) out.push(normalizeMeta(m, id));
        }
      }
    } catch {}
    // Do not use a generic upcoming catalog here. If the league feed is empty,
    // use the exact ESPN league endpoint instead.
...
function filtered(metas, mode) {
  ...
  if (mode === "ufc") return metas.filter(m => /ufc|mma/.test(classify(m).t));
  return metas;
}
function manifest() {
...
async function proxy(req, res, path) { const r = await fetch(`http://127.0.0.1:${UPSTREAM_PORT}${path}`, { method: req.method, headers: { accept: req.headers.accept || "*/*" } }); res.writeHead(r.status, { "content-type": r.headers.get("content-type") || "application/octet-stream", "cache-control": "no-store", "access-control-allow-origin": "*" }); res.end(Buffer.from(await r.arrayBuffer())); }
