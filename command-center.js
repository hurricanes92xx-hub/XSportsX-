...
// so encrypted configuration tokens remain valid after service restarts.
const CONFIG_SECRET=process.env.XSPORTSX_CONFIG_SECRET||crypto.randomBytes(32).toString("hex");
const CONFIG_KEY=crypto.createHash("sha256").update(CONFIG_SECRET).digest();
...
const NEWS_NETWORKS=[["ESPN","ESPN"],["ESPN2","ESPN2"],["ESPNU","ESPNU"],["ESPN News","ESPNEWS"],["ESPN Deportes","ESPN Deportes"],["ACC Network","ACCN"],["SEC Network","SECN"],["Big Ten Network","BTN"],["NFL Network","NFL Network"],["MLB Network","MLB Network"],["NBA TV","NBA TV"],["NHL Network","NHL Network"],["CBS Sports Network","CBS Sports"],["FS1","FS1"],["FS2","FS2"],["Fox Sports","Fox Sports"],["Golf Channel","Golf Channel"],["TNT Sports","TNT Sports"],["NBC Sports","NBC Sports"],["beIN Sports","beIN Sports"],["Sportsnet","Sportsnet"],["Tennis Channel","Tennis Channel"]];
...
function decryptConfig(token){if(!token)return null;try{const raw=Buffer.from(token,"base64url");if(raw.length<29)return null;const decipher=crypto.createDecipheriv("aes-256-gcm",CONFIG_KEY,raw.subarray(0,12));decipher.setAuthTag(raw.subarray(12,28));const v=JSON.parse(Buffer.concat([decipher.update(raw.subarray(28)),decipher.final()]).toString("utf8"));return v?.server&&v?.username&&v?.password?{server:String(v.server).replace(/\/$/,""),username:String(v.username),password:String(v.password)}:null;}catch{return null;}}
function requestConfig(u){const parts=u.pathname.split("/").filter(Boolean),pIndex=parts.indexOf(PREFIX.slice(1));if(pIndex>=0){const token=parts[pIndex+1];if(token&&!["configure","manifest.json","health","xtream"].includes(token))return decryptConfig(token);}return decryptConfig(u.searchParams.get("config"));}
function routePath(u){const parts=u.pathname.split("/").filter(Boolean),p=parts.indexOf(PREFIX.slice(1));if(p<0)return u.pathname;const after=parts.slice(p+1);if(!after.length)return "/";if(after[0]==="configure")return "/configure";if(after[0]==="manifest.json")return "/manifest.json";if(after[0]==="health")return "/health";if(after[0]==="xtream")return "/"+after.join("/");if(after.length>=2)return "/"+after.slice(1).join("/");return "/"+after[0];}
...
function configurePage(){return `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>XSportsX IPTV Setup</title><style>body{margin:0;background:#07090d;color:#fff;font:16px system-ui;display:grid;place-items:center;min-height:100vh}main{width:min(520px,92vw);background:#11151d;padding:28px;border:1px solid #252c38;border-radius:18px}label{display:block;margin:16px 0 7px}input{width:100%;box-sizing:border-box;padding:13px;border-radius:10px;border:1px solid #343d4d;background:#090c12;color:#fff}button{width:100%;margin-top:22px;padding:14px;border:0;border-radius:10px;background:#e21d2d;color:#fff;font-weight:700;font-size:16px}</style></head><body><main><h1>📡 XSportsX IPTV</h1><p>Enter the Xtream details from your authorized provider. Credentials are submitted over HTTPS and are never placed in the browser URL.</p><form method="post" action="${PREFIX}/configure"><label>Xtream Server URL</label><input name="server" required placeholder="https://provider.example:8080"><label>Username</label><input name="username" required autocomplete="username"><label>Password</label><input type="password" name="password" required autocomplete="current-password"><button type="submit">GENERATE NUVIO MANIFEST</button></form></main></body></html>`;}
function readyPage(manifestUrl){return `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>XSportsX Ready</title><style>body{margin:0;background:#07090d;color:#fff;font:16px system-ui;display:grid;place-items:center;min-height:100vh}main{width:min(560px,92vw);background:#11151d;padding:28px;border:1px solid #293241;border-radius:18px}input{width:100%;box-sizing:border-box;padding:13px;margin:8px 0 14px;background:#090c12;color:#fff;border:1px solid #343d4d;border-radius:10px}a{display:block;width:100%;box-sizing:border-box;padding:14px;margin-top:12px;border-radius:10px;background:#e21d2d;color:#fff;text-align:center;text-decoration:none;font-weight:700}</style></head><body><main><h1>✅ XSportsX is configured</h1><p>Your Nuvio-ready manifest is ready. Copy this URL into Nuvio → Addons → Add Addon.</p><input readonly value="${esc(manifestUrl)}"><a href="${esc(manifestUrl)}">OPEN MANIFEST</a></main></body></html>`;}

const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url||"/",`http://${req.headers.host}`),path=routePath(u),c=requestConfig(u);
if(path==="/configure"){
  ...
  if(req.method==="POST"){
    ...
    const form=new URLSearchParams(body),serverUrl=String(form.get("server")||"").trim().replace(/\/$/,""),username=String(form.get("username")||"").trim(),password=String(form.get("password")||"");
    if(!/^https?:\/\//i.test(serverUrl)||!username||!password){res.writeHead(400,{"content-type":"text/plain; charset=utf-8"});return res.end("Invalid Xtream configuration");}
    const cfg={server:serverUrl,username,password};
    try{const info=await xtreamFetch(cfg);if(info?.user_info?.auth!==1)throw new Error("Xtream authentication failed");}
    catch(e){res.writeHead(401,{"content-type":"text/html; charset=utf-8","cache-control":"no-store"});return res.end(`<p>Xtream login failed: ${esc(e?.message||e)}</p><p><a href="${PREFIX}/configure">Go back</a></p>`);}
    const manifestUrl=configManifestUrl(cfg);res.writeHead(200,{"content-type":"text/html; charset=utf-8","cache-control":"no-store"});return res.end(readyPage(manifestUrl));
  }
  res.writeHead(405,{"allow":"GET, POST"});return res.end();
}
if(path==="/manifest.json")return json(res,manifest());
if(path==="/health")return json(res,{ok:true,version:VERSION,addonId:ADDON_ID,xtream:true,credentialTransport:"aes-256-gcm-token",stableConfigSecret:!!process.env.XSPORTSX_CONFIG_SECRET,nuvioCompatible:true,sportsNewsNetworks:NEWS_NETWORKS.map(x=>x[1])});
if(path==="/xtream/status.json"){try{const x=await loadXtream(c);return json(res,{configured:x.configured,user:x.user||null,categories:x.categories?.length||0,channels:x.metas?.length||0,sportsNewsChannels:x.sportsNews?.length||0,error:x.error||null})}catch(e){return json(res,{configured:!!c,ok:false,error:String(e?.message||e)},401)}}
if(path==="/catalog/channel/iptv-live.json"){try{const x=await loadXtream(c);return json(res,{metas:x.metas||[],error:x.error||null})}catch(e){return json(res,{metas:[],error:String(e?.message||e)},401)}}
if(path==="/catalog/channel/sports-news.json"){try{const x=await loadXtream(c);return json(res,{metas:x.sportsNews||[],error:x.error||null})}catch(e){return json(res,{metas:[],error:String(e?.message||e)},401)}}
if(path.startsWith("/meta/channel/xtream:")){try{const x=await loadXtream(c),id=decodeURIComponent(path.slice(path.indexOf("xtream:")).replace(/\.json$/,"")),meta=x.metas.find(m=>m.id===id)||null;return json(res,{meta},meta?200:404)}catch(e){return json(res,{meta:null,error:String(e?.message||e)},401)}}
if(path.startsWith("/stream/channel/xtream:")){try{const x=await loadXtream(c),id=decodeURIComponent(path.slice(path.indexOf("xtream:")).replace(/\.json$/,"")),meta=x.metas.find(m=>m.id===id);return json(res,{streams:meta?.xtream?.streamUrl?[{name:`▶ ${meta.name}`,url:meta.xtream.streamUrl,title:meta.name}]:[]})}catch(e){return json(res,{streams:[],error:String(e?.message||e)},401)}}
if(path.startsWith("/catalog/channel/")){const id=path.slice(17).replace(/\.json$/,""),ms=await loadSports();return json(res,{metas:filterSports(ms,id==="sports-command-center"?"featured":id==="sports-epg"?"all":id)})}
if(path.startsWith("/meta/channel/")){const id=decodeURIComponent(path.slice(14).replace(/\.json$/,"")),ms=await loadSports(),meta=ms.find(m=>m.id===id)||null;return json(res,{meta},meta?200:404);}
return json(res,{error:"Not found"},404);
}catch(e){console.error("XSportsX request error",e);return json(res,{error:String(e?.message||e)},502);}});
