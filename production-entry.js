...
// 4.4.0 production redeploy marker: force hosting platforms to rebuild the authoritative gateway.
...
function commandCenterCatalog(metas, mode) {
  ...
  return result.map(m => ({ ...m, type: "channel", posterShape: "landscape", genres: [...new Set([...(m.genres || []), "Sports Command Center", mode])], behaviorHints: { ...(m.behaviorHints || {}), defaultVideoId: m.behaviorHints?.defaultVideoId || m.videos?.[0]?.id } })).slice(0, 500);
}

async function upstream(path, init) { return fetch(`http://127.0.0.1:${UPSTREAM_PORT}${path}`, init); }
async function upstreamJson(path) { const response = await upstream(path); if (!response.ok) throw new Error(`upstream ${response.status}`); return response.json(); }

function buildManifest() {
  return { id: ADDON_ID, version: VERSION, name: "XSportsX Sports Command Center", description: "Ultimate sports command center with live-first game discovery, starting-soon alerts, featured matchups, league hubs, UFC and NCAA Football intelligence, broadcast networks, artwork, EPG and stream resolution.", logo: "https://a.espncdn.com/i/teamlogos/leagues/500/nfl.png", background: "https://a.espncdn.com/i/teamlogos/leagues/500/nfl.png", resources: [{ name: "catalog", types: ["channel"] }, { name: "meta", types: ["channel"], idPrefixes: ["sport:"] }, { name: "stream", types: ["channel"], idPrefixes: ["sport:"] }], types: ["channel"], idPrefixes: ["sport:"], behaviorHints: { configurable: false, configurationRequired: false }, liveTv: { enabled: true, name: "XSportsX Sports Live TV", playlist: `${BASE}/live-tv.m3u`, epg: `${BASE}/epg.xml`, guide: `${BASE}/catalog/channel/sports-command-center.json`, refreshSeconds: 60 }, catalogs: [{ type: "channel", id: "sports-command-center", name: "🏆 XSPORTSX • SPORTS COMMAND CENTER" }, { type: "channel", id: "live-now", name: "🔴 LIVE NOW" }, { type: "channel", id: "starting-soon", name: "⏰ STARTING SOON" }, { type: "channel", id: "featured", name: "⭐ FEATURED" }, { type: "channel", id: "sports-epg", name: "📺 ALL SPORTS • EPG" }, { type: "channel", id: "nfl", name: "🏈 NFL" }, { type: "channel", id: "ncaaf", name: "🏈 NCAA FOOTBALL" }, { type: "channel", id: "nba", name: "🏀 NBA" }, { type: "channel", id: "nhl", name: "🏒 NHL" }, { type: "channel", id: "mlb", name: "⚾ MLB" }, { type: "channel", id: "ufc", name: "🥊 UFC COMMAND CENTER" }, { type: "channel", id: "soccer", name: "⚽ SOCCER" }] };
}
...
async function proxy(req, res) {
  ...
  if (path.startsWith("/catalog/channel/")) { const id = path.slice("/catalog/channel/".length).replace(/\.json$/, ""); const source = await upstreamJson("/catalog/channel/sports-epg.json").catch(async () => upstreamJson("/catalog/sport/upcoming.json")); if (id === "sports-command-center" || id === "sports-epg") return send(res, 200, JSON.stringify({ metas: commandCenterCatalog(source.metas || [], id === "sports-epg" ? "all" : "featured") })); if (["live-now","starting-soon","featured","nfl","ncaaf","nba","nhl","mlb","ufc","soccer"].includes(id)) return send(res, 200, JSON.stringify({ metas: commandCenterCatalog(source.metas || [], id) })); }
  ...
  const type = upstreamResponse.headers.get("content-type") || "application/octet-stream", body = Buffer.from(await upstreamResponse.arrayBuffer()); res.writeHead(upstreamResponse.status, headers(type)); res.end(body);
}
