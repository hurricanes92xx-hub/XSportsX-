...
async function proxy(req, res) {
  ...
  if (path === "/manifest.json" || path === "/manifest-4.3.1.json") {
    const upstream = await fetch(`http://127.0.0.1:${UPSTREAM_PORT}/manifest.json`);
    const manifest = await upstream.json();
    manifest.id = ADDON_ID; manifest.version = VERSION; manifest.name = "XSportsX Sports EPG";
    manifest.catalogs = [
      { type: "channel", id: "sports-epg", name: "📺 SPORTS EPG • ALL GAMES & NETWORKS" },
      { type: "channel", id: "sports-guide", name: "🗓️ SPORTS GUIDE • NOW & NEXT" }
    ];
    manifest.liveTv = { ...(manifest.liveTv || {}), enabled: true, name: "XSportsX Sports Live TV", playlist: "/live-tv.m3u", epg: "/epg.xml", refreshSeconds: 60, guide: "/catalog/channel/sports-guide.json" };
    return send(res, 200, JSON.stringify(manifest));
  }
  if (path === "/health") return send(res, 200, JSON.stringify({ ok: true, version: VERSION, addonId: ADDON_ID, type: "channel", liveTv: true }));
  if (path === "/live-tv.json") return send(res, 200, JSON.stringify({ id: ADDON_ID, version: VERSION, name: "XSportsX Sports Live TV", playlist: `${BASE}/live-tv.m3u`, epg: `${BASE}/epg.xml`, guide: `${BASE}/catalog/channel/sports-guide.json`, refreshSeconds: 60 }));
  if (path === "/catalog/channel/sports-guide.json") {
    const paths = ["/catalog/sport/live-now.json", "/catalog/sport/starting-soon.json", "/catalog/sport/upcoming.json", "/catalog/sport/cfp-watch.json"];
    const feeds = await Promise.all(paths.map(async p => { try { const r = await fetch(`http://127.0.0.1:${UPSTREAM_PORT}${p}`); return r.ok ? await r.json() : { metas: [] }; } catch { return { metas: [] }; } }));
    const byId = new Map();
    for (const feed of feeds) for (const meta of feed.metas || []) if (meta?.id) byId.set(meta.id, { ...meta, type: "channel", posterShape: "landscape", genres: [...new Set([...(meta.genres || []), "Sports Guide", "EPG"])] });
    return send(res, 200, JSON.stringify({ metas: [...byId.values()].slice(0, 500) }));
  }
