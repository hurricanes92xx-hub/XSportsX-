...
async function ufcRankingsCatalog() {
  ...
  const metas = rows.slice(0, 150).map((fighter, index) => ({ id: `sport:ufc-ranking-${index}-${norm(fighter.name).replace(/ /g, "-")}`, type: "sport", name: `🏆 ${fighter.rank ? `#${fighter.rank} ` : ""}${fighter.name}`, poster: fighter.image || "https://www.ufc.com/themes/custom/ufc/assets/img/ufc-logo.svg", background: "https://www.ufc.com/themes/custom/ufc/assets/img/ufc-logo.svg", description: `Official UFC Rankings • ${fighter.division || "Rankings"}\n\nOpen official UFC fighter profile for current stats and details.`, genres: ["Sports", "UFC", "Rankings"], videos: [{ id: `sport:ufc-ranking-${index}-${norm(fighter.name).replace(/ /g, "-")}`, title: `🏆 ${fighter.name}`, overview: `Official UFC profile: ${fighter.profileUrl}`, thumbnail: fighter.image }], behaviorHints: {} }));
...
async function ufcFightersCatalog() {
  ...
  const metas = rows.slice(0, 150).map((fighter, index) => ({ id: `sport:ufc-fighter-${index}-${norm(fighter.name).replace(/ /g, "-")}`, type: "sport", name: `👊 ${fighter.name}`, poster: fighter.image || "https://www.ufc.com/themes/custom/ufc/assets/img/ufc-logo.svg", background: "https://www.ufc.com/themes/custom/ufc/assets/img/ufc-logo.svg", description: `Official UFC athlete profile\n\n${fighter.profileUrl}`, genres: ["Sports", "UFC", "Fighters"], videos: [{ id: `sport:ufc-fighter-${index}-${norm(fighter.name).replace(/ /g, "-")}`, title: `👊 ${fighter.name}`, overview: `Official UFC athlete profile: ${fighter.profileUrl}`, thumbnail: fighter.image }], behaviorHints: {} }));
...
const server = http.createServer(async (req, res) => {
  try {
    const isUfc = /\/(catalog|meta|stream)\/sport\/ufc(?:\.json|-event-|-(?:rankings|fighters)\.json)/.test(req.url || "");
