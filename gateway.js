...
async function ufcCatalog() {
  ...
  const [data, backend] = await Promise.all([
    getUfcData(),
    backendJson("/catalog/sport/ufc.json").catch(() => ({ metas: [] }))
  ]);
  const metas = Array.isArray(backend.metas) ? backend.metas : [];

  // Never remove the working UFC catalog just because the optional enrichment source is unavailable.
  if (!data.length) {
    catalogCache = { at: Date.now(), value: metas };
    return metas;
  }
...
async function handleUfc(req, res) {
  ...
  if (url.pathname.startsWith("/stream/sport/ufc-event-")) {
    ...
    const streams = sourceVideo?.id
      ? await backendJson(`/stream/sport/${encodeURIComponent(sourceVideo.id.replace(/^sport:/, ""))}.json`).catch(() => ({ streams: [] }))
      : { streams: [] };
    const result = Array.isArray(streams.streams) ? [...streams.streams] : [];
    const enriched = await getUfcData();
    const officialUrl = enriched.find(x => `sport:ufc-event-${x.id}` === id)?.officialUrl;
