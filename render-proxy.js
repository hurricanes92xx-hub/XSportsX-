...
const COMPAT_PREFIXES=["v520","v516","v516","v515","v514","v512","v511","v510","v509","v508","v507","v506","v505","v504","v503","v502","v501","v500"];
...
const child=spawn(process.execPath,["sports-router-v521.js"],{
  env:{...process.env,PORT:String(INTERNAL_PORT),XSPORTSX_CONFIG_SECRET:CONFIG_SECRET},
  stdio:"inherit"
});
...
function proxyPath(raw){
  ...
  if(["configure","manifest.json","health"].includes(rest[0]))return{path:"/"+rest[0],query:u.search};
  if(rest[0]==="assets")return{path:"/"+rest.map(decode).join("/"),query:u.search};
  const token=decode(rest[0]),resource=rest.slice(1).map(decode),q=new URLSearchParams(u.search);q.set("config",token);
