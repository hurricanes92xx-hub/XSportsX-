...
const baseManifest={id:"com.xsportsx.sports.epg.v530",version:VERSION,name:"XSportsX Sports Command Center",description:"Fast sports EPG with live sports and authorized Xtream source resolution.",resources:[{name:"catalog",types:["channel"]},{name:"meta",types:["channel"]},{name:"stream",types:["channel"]}],types:["channel"],idPrefixes:["sport:","xtream:","news:","live:"],catalogs:catalogs.map(([id,name])=>({type:"channel",id,name})),behaviorHints:{configurable:true,configurationRequired:true}};
...
const server=http.createServer((req,res)=>{
 try{
  ...
  const up=http.request(target,{method:req.method,headers:{...req.headers,host:`127.0.0.1:${INTERNAL_PORT}`,connection:"keep-alive"}},ur=>{
   let data="";ur.setEncoding("utf8");ur.on("data",x=>data+=x);ur.on("end",()=>{const h={...ur.headers,"x-xsportsx-version":VERSION};delete h["content-length"];res.writeHead(ur.statusCode||502,h);res.end(data);});
  });
