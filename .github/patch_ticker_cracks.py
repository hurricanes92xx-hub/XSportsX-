...
# Mobile: thicken the existing XSportsX crack treatment.
...
if 'private fun TvGlowingCracks(' not in ts:
    ts = ts.replace('Box(Modifier.fillMaxSize().background(TvBg)) {\n        Row(Modifier.fillMaxSize()) {', 'Box(Modifier.fillMaxSize().background(TvBg)) {\n        TvGlowingCracks(Modifier.fillMaxSize())\n        Row(Modifier.fillMaxSize()) {', 1)
...
# Bump signed APK version for the in-app update path.
gradle = Path('app/build.gradle.kts')
gs = gradle.read_text().replace('versionCode = 14', 'versionCode = 15', 1).replace('versionName = "1.6.3"', 'versionName = "1.6.4"', 1)
