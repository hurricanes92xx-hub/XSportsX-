#!/usr/bin/env python3
"""Run the canonical refresh with conservative Google-backed gap discovery."""
...
# Promote only candidates that have survived repeated probes before they become
# normal providers. This is deliberately applied before the next refresh.
...
def _build_matrix(*args,**kwargs):
    ...
    for league in list(matrix):
        ...
        if promoted:
            ...
            if "discovery" not in configured:
...
def _observe_known():
    # Re-probe durable candidates so a provider earns promotion through repeated
    # real observations instead of a single lucky search result.
    try:
...
def main():
    ...
    discovered_events=[]
    for league in gaps:
        # Search only where the canonical matrix actually has a gap. This keeps
        # Google out of the hot path for healthy leagues.
        candidates=discovery.discover(league,max_queries=2)
    ...
    if discovered_events:
        ...
        remaining=[l for l in gaps if not any(e.get("league")==l for e in discovered_events)]
        payload["noEventLeagues"]=remaining
        payload["eventCounts"]={}
    ...
    tmp.replace(OUT)
    print(f"adaptive discovery: candidates={discovered}; promoted={promoted}; fallback_attempts={fallback_attempts}")
