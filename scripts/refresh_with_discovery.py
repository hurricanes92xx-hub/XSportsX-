...
def main():
    ...
    if discovered_events:
        canonical,merges,_=core.dedupe((payload.get("events") or [])+discovered_events);canonical.sort(key=lambda e:e.get("start") or e.get("startUtc") or "");payload["events"]=canonical;payload["identityMergeCount"]=int(payload.get("identityMergeCount",0))+merges;payload["noEventLeagues"]=[l for l in gaps if not any(e.get("league")==l for e in discovered_events)];payload["eventCounts"]={}
