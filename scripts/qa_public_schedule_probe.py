#!/usr/bin/env python3
"""Probe the public ESPN schedule endpoints used by XSportsX.

The probe is diagnostic: it records reachable/unreachable endpoints without
preventing the emulator from running. Zero-event leagues are allowed because
seasonal sports can legitimately have no games in the active window.
"""
import json
import sys
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "XSportsX-QA/1.1", "Accept": "application/json"})
    ...
    for name, path in LEAGUES:
        ...
        for base in BASES:
            ...
            try:
                ...
                events = max(events, len(raw))
                ok = True
                break
            except Exception as exc:
                errors.append(f"{base.split('/apis/')[1]}: {exc}")
        row = {"league": name, "events_3d": events, "ok": ok, "errors": errors}
        results.append(row)
        if not ok:
            failures.append(row)
        print(f"PUBLIC {name:18} {'OK':3} events={events}" if ok else f"PUBLIC {name:18} GAP {errors}")
