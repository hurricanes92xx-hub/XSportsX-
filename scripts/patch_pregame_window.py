...
# service remains focused on fetching/normalizing schedule data. The check is
# whitespace-insensitive so repeated CI runs never add duplicate methods.
if model.exists():
    text = model.read_text()
    if not re.search(r'\bfun\s+isPregame\s*\(', text):
