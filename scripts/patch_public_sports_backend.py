...
SPORT_RE = r"const SPORT_RE=/\\b(sport|sports|espn|espn\\+|fox sports|fs1|fs2|tnt|nba|mlb|nhl|nfl|ncaaf|ncaab|wnba|sec network|acc network|big ten|bally|msg|regional sports|baseball|basketball|football|hockey|soccer|tennis|golf|cbs sports|nbc sports|bein|sky sport|f1|formula|racing|ufc|boxing|fight|paramount|tsn|sportsnet|peacock|fanatiz|rugby|volleyball|lacrosse|wrestling|judo|taekwondo|swimming|diving|water polo|gymnastics|cycling|darts|snooker|archery|equestrian|handball|field hockey|cricket|monster jam|motogp|wrc|wec|imsa|formula e|mxgp|esports|gaming)\\b/i;"

ALIASES = """const SPORT_ALIASES={
...
OFFICIAL_SOURCES = """const OFFICIAL_SOURCES={rugby:['RugbyPass TV','https://www.rugbypass.tv/'],monsterjam:['Monster Jam','https://www.monsterjam.com/en-us/watch/'],motogp:['MotoGP','https://www.youtube.com/@MotoGP'],wrc:['FIA World Rally Championship','https://www.youtube.com/@WorldRallyChampionship'],wec:['FIA WEC','https://www.youtube.com/@FIAWEC'],imsa:['IMSA','https://www.youtube.com/@IMSAOfficial'],formulae:['Formula E','https://www.youtube.com/@FIAFormulaE'],mxgp:['MXGP','https://www.youtube.com/@mxgptv'],esports:['Red Bull TV','https://www.redbull.com/us-en/live-events']};
function sourceForLeague(k){const x=OFFICIAL_SOURCES[k];return x?{name:x[0],url:x[1]}:null;}"""


def patch_server(path: Path):
    s=path.read_text()
    if 'XSportsX public sports backend v2' in s:
        return
    s=re.sub(r"const LEAGUES=\{.*?\n\};", LEAGUES, s, count=1, flags=re.S)
    s=re.sub(r"const SPORT_RE=.*?;", SPORT_RE, s, count=1, flags=re.S)
    s=re.sub(r"function selectedLeagueSet\(c\)\{.*?\nfunction config", ALIASES+"\nfunction config", s, count=1, flags=re.S)
    marker="function scoreboardUrl(k){const l=LEAGUES[k];return l?`https://site.api.espn.com/apis/site/v2/sports/${l[1]}/${l[2]}/scoreboard?dates=${dateRange()}&limit=100`:'';}"
    if marker in s:
        s=s.replace(marker, marker+"\n"+OFFICIAL_SOURCES, 1)
    s=s.replace("genres:['Sports',e.leagueName],sportSource:e.league,eventId:e.id,event:e,behaviorHints:{isPlayable:true}", "genres:['Sports',e.leagueName],sportSource:e.league,eventId:e.id,event:e,officialSource:sourceForLeague(e.league),behaviorHints:{isPlayable:true}", 1)
    s="// XSportsX public sports backend v2\n"+s
    path.write_text(s)


def patch_bootstrap(path: Path):
    s=path.read_text()
    if 'XSportsX public sports backend v2' in s:
        return
    s=re.sub(r"const LEAGUES=\{.*?\n\};", LEAGUES, s, count=1, flags=re.S)
    s=s.replace("// XSPORTSX public sports backend v2", "")
    s="// XSportsX public sports backend v2\n"+s
    path.write_text(s)

patch_server(Path('server.js'))
patch_bootstrap(Path('bootstrap.js'))
print('Expanded backend league/event metadata and official-source hints.')
