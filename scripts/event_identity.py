...
def _extract_teams(event,league):
    home=event.get("home") or event.get("homeTeam") or ""; away=event.get("away") or event.get("awayTeam") or ""
    if home or away:return _team(home,league),_team(away,league)
    raw=str(event.get("title") or "").lower()
    parts=re.split(r"\s+@\s+|\s+at\s+|\s+vs\.?\s+|\s+versus\s+",raw)
...
def provider_identity(provider,provider_event_id):
    provider=normalize_text(provider); provider_event_id=str(provider_event_id or "").strip()
    return f"{provider}:{provider_event_id}" if provider and provider_event_id else ""

def event_identity(league,title,start):
