...
def event_identity(league: object, title: object, start: object) -> str:
    """Return a compact deterministic ID for the logical game/event.

    Provider IDs are intentionally not required: this identity remains stable
    when the same event arrives from ESPN, an official API, or a repair source.
    """
