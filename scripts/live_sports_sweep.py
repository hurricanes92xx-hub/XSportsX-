...
HEADERS={"User-Agent":"XSportsX-LiveSweep/1.1","Accept":"application/json, */*","Accept-Language":"en-US,en;q=0.9"}
...
def main():
    ...
    by_provider={};live_provider=final_provider=fetched_events=0;failed=[]
    ...
    for pid,fresh in by_provider.items():
        ...
        fresh["sport"]=str(next((m[1] for m in ESPN_LEAGUES if m[0]==fresh.get("league")),"other")).lower();fresh["id"]=event_identity(fresh.get("league"),fresh.get("title"),fresh.get("startUtc"),fresh.get("home"),fresh.get("away"));events.append(fresh);added+=1
    inferred=0
    for event in events:
    ...
    events.sort(key=lambda e:str(e.get("startUtc") or e.get("start") or ""));payload["events"]=events;payload["liveSweep"]={"schema":1,"checkedAtUtc":now.isoformat().replace("+00:00","Z"),"leaguesChecked":len(ESPN_LEAGUES),"providerEventsFetched":fetched_events,"providerLive":live_provider,"providerFinal":final_provider,"stateChanges":changed,"eventsAdded":added,"boundedTimingPromotions":inferred,"failedLeagues":sorted(failed),"liveCountAfterSweep":sum(1 for e in events if e.get("tag")=="LIVE")};payload["eventCounts"]={}
    for event in events:
