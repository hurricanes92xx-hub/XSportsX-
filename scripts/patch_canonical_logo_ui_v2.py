...
for path in required:
    if not path.exists(): raise SystemExit(f'missing required source: {path}')
logo_text = required[0].read_text(encoding='utf-8')
if 'XSportsLeagueLogo' not in logo_text or 'XSportsNetworkLogo' not in logo_text: raise SystemExit('shared sports logo renderer is missing')
