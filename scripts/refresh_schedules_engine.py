...
season=load('xsportsx_season_intelligence',ROOT/'scripts'/'season_intelligence.py')
MAX_CONCURRENT=8
semaphore=threading.BoundedSemaphore(MAX_CONCURRENT)
guards={}; guards_lock=threading.Lock()
def guard_for(url):
...
def previous_events():
    try: return (json.loads((ROOT/'data/schedule_feed.json').read_text(encoding='utf-8')).get('events') or [])
    except Exception: return []
PREVIOUS=previous_events(); SEASON_REPORT=[]
_original_add_espn=refresh.add_espn; _original_add_ncaa=refresh.add_ncaa
...
def season_aware_espn(events,name,sport,league,icon,days):
    d=season.analyze(name,PREVIOUS); SEASON_REPORT.append(d|{'provider':'espn'})
...
def season_aware_ncaa(events,name,sport,division,icon,days=30):
    d=season.analyze(name,PREVIOUS); SEASON_REPORT.append(d|{'provider':'ncaa'})
    if not d['active']: return True,preserved(events,name)
    return _original_add_ncaa(events,name,sport,division,icon,days)
refresh.get=guarded_get
refresh.add_espn=season_aware_espn
refresh.add_ncaa=season_aware_ncaa
refresh.main()
