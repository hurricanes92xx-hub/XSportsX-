#!/usr/bin/env python3
"""Generate policy for background sports-API corroboration.

Actual API retrieval belongs to the backend refresh worker; the playback/UI path
must consume only cached data. This manifest prevents accidental live API calls.
"""
import json
...
def main():
    ...
    payload = {
        ...
        'fallbackOrder': ['existing schedule', 'ESPN', 'TheSportsDB', 'EPG', 'Google', 'Telegram'],
        'rule': 'Use cached corroboration only; never call an external metadata API from the Play path.'
    }
    OUT.write_text(json.dumps(payload, indent=2) + '\n', encoding='utf-8')
    print(f'wrote sports API refresh policy for {len(LEAGUES)} catalog leagues')
