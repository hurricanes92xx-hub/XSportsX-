...
def iso(v):
    x=dt(v)
    return x.isoformat().replace('+00:00','Z') if x else None
...
def state(event):
    c=(event.get('competitions') or [{}])[0]
    s=((c.get('status') or {}).get('type') or {}) or ((event.get('status') or {}).get('type') or {})
...
def find_match(events,league,name,start):
    remote=dt(start)
    tokens=parts(name)
    candidates=[e for e in events if e.get('league')==league]
    ...
    if exact:
        ...
        for e in candidates:
            ...
            if remote and old and abs((old-remote).total_seconds())<=10800:
                ...
                if shared:
                    near.append((shared,-abs((old-remote).total_seconds()),e))
        if near:
            return sorted(near,key=lambda x:(x[0],x[1]),reverse=True)[0][2]
        return None
    if len(candidates)==1:
        return candidates[0]
...
def main():
    ...
    changed=added=failures=0
    today=datetime.now(timezone.utc).date()
    for league,sport,slug in ESPN_LEAGUES:
        try:
            root=get_json(f'https://site.api.espn.com/apis/site/v2/sports/{sport}/{slug}/scoreboard?dates={today:%Y%m%d}&limit=1000')
        ...
        for remote in root.get('events') or []:
            ...
            if not start:
                continue
            name=title(remote,league)
            tag=state(remote)
            if league=='MLB' and tag!='FINAL' and mlb_time_fallback(start,tag):
                tag='LIVE'
            ...
            if event is not None:
                ...
                events.append({'league':league,'title':name,'start':start,'tag':'LIVE','icon':'•','source':'espn','sourceDetail':'ESPN live-status reconciliation'})
                added+=1
    ...
    if changed or added:
        report=payload.setdefault('repairReport',{})
        report['liveStatusReconciled']=changed
        report['liveEventsAdded']=added
        report['liveProviderFailures']=failures
    ...
    if failures:
        raise SystemExit(f'{failures} ESPN scoreboard providers failed')
