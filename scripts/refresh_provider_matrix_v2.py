#!/usr/bin/env python3
"""Canonical schedule refresh using primary/secondary/tertiary plus cache recovery."""
...
def icon_map():
    out={x[0]:x[3] for x in engine.ESPN_LEAGUES}; out.update({x[0]:x[3] for x in engine.NCAA_LEAGUES})
    out.update({x:"🏎️" for x in engine.NASCAR_SERIES}); out.update({"WWE":"🏆","AEW":"🤼","TNA":"🤼"}); return out

def dedupe(events):
    priority={"official":0,"ncaa":1,"nascar":1,"espn":2,"espn-ncaa":2,"sportsdb":3,"fallback":4,"cache":5}; canonical=[]; merges=0; counts={}
    for raw in events:
...
def fetch(provider,league,meta,official,previous):
    try:
        ...
        if provider=="espn":
            ...
            events=[]; ok,_=engine.add_espn(events,league,*row); return bool(ok and events),events,""
        if provider=="ncaa":
            row=meta.get("ncaa")
            if not row:return False,[],"not configured"
            events=engine.fetch_ncaa_league(league,*row,horizon_days=30); return bool(events),events,""
        if provider=="espn-ncaa":
            ...
            return bool(ok and events),events,""
        if provider=="nascar":
            events=engine.fetch_nascar_league(league,horizon_days=370); return bool(events),events,""
        if provider=="sportsdb":
            events=[]; ok,_=engine.add_sportsdb(events,league,meta["icon"]); return bool(ok and events),events,""
        if provider=="cache":
...
def main():
    ...
    previous_events=previous.get("events") or []; officials=official_map(); icons=icon_map()
    espn={x[0]:x[1:] for x in engine.ESPN_LEAGUES}; ncaa={x[0]:x[1:] for x in engine.NCAA_LEAGUES}
    dedicated_names=set(ncaa)|set(engine.NASCAR_SERIES); official_names=set(officials); sportsdb_names=set(engine.SPORTDB_LEAGUES)
    ...
    for league in sorted(all_leagues):
        ordered=provider_order(league,matrix[league]["configured"]); matrix[league]["activeOrder"]=ordered; attempts[league]=[]; selected=None
        meta={"icon":icons.get(league,"🏆"),"espn":espn.get(league),"ncaa":ncaa.get(league)}
        # Live providers occupy primary/secondary/tertiary. Cache is a fourth, recovery-only step.
    ...
    # Wrestling is an additional authority; its own official/fallback data is canonicalized below.
    ...
    payload={"schema":9,"generatedAt":datetime.now(timezone.utc).isoformat(),"refreshHours":6,"eventCounts":per,"failedSources":failures,"providerFailures":failures,"sportsDbFallbackSources":[],"officialSourceFailures":[],"officialSourceCounts":{},"identityMergeCount":merges,"sourceRecordCounts":source_counts,"leagueProviderMatrix":matrix,"providerAttempts":attempts,"providerPromotions":promotions,"cacheRecoveryLeagues":cache_recovery,"events":events}
    tmp=OUT.with_suffix(".tmp"); tmp.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+"\n",encoding="utf-8"); tmp.replace(OUT)
    print(f"wrote {len(events)} canonical events across {len(per)} leagues; matrix={len(matrix)}; promotions={len(promotions)}; cache={len(cache_recovery)}; identity_merges={merges}")
