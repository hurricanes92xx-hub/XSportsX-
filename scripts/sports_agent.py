...
def _recover_html_schedule(league,url):
    ...
    for link in links:
        slug=link.group(1);title=' '.join(re.sub(r'<[^>]+>',' ',link.group(2)).split()).strip()
        if not title or slug in seen:continue
        m=date_re.search(text[link.start():min(len(text),link.end()+9000)])
        if not m:continue
        try:dt=datetime.strptime(f'{m.group(1)} {m.group(2)} {now.year} {m.group(3)}','%b %d %Y %I:%M %p')
        except Exception:continue
        offsets={'UTC':0,'GMT':0,'EST':-5,'EDT':-4,'CST':-6,'CDT':-5,'MST':-7,'MDT':-6,'PST':-8,'PDT':-7};tz=m.group(4).upper()
        if tz not in offsets:continue
        dt=dt.replace(tzinfo=timezone(timedelta(hours=offsets[tz])));start=dt.astimezone(timezone.utc)
...
class ToolRegistry:
    ...
    def discover_schedule_provider(self,e):
        ...
        candidates=discovery.discover(e.league,max_queries=4);promoted=discovery.promote_successful(e.league);events=discovery.discovery_events(e.league);research=web_research.research_schedule(e.league,{'title':e.title,'startUtc':e.start_utc},limit=8);recovered=[]
    ...
    def discover_event_source_metadata(self,e):
        ...
        return {'status':'completed','league':e.league,'candidates':len(candidates),'eventMatches':matches[:8],'googleLiveResearch':web_research.research_live({'title':e.title,'league':e.league,'startUtc':e.start_utc},limit=10)}
    def probe_live_state_and_source(self,e):
        ...
        return out
    def refresh_live_evidence(self,e):return {'status':'completed','evidenceRefresh':True,'correlation':e.correlated or {},'probe':self.probe_live_state_and_source(e),'googleLiveResearch':self.discover_event_source_metadata(e) if e.phase in {'LIVE','PREGAME'} or not e.source_present else None}
...
def run(feed_path,memory_path,graph_path,mode='full'):
    ...
    if mode=='full':
        for league in _mandatory_schedule_gaps(feed):
            ...
            if recovered:events,merges,_=dedupe_events(events+recovered);feed['identityMergeCount']=int(feed.get('identityMergeCount',0))+merges
        feed['events']=events
