...
# All providers feed one canonical identity/merge layer before publication.
import json
import re
...
def dedupe_and_merge(events):
    # Explicit order: official/dedicated authority > ESPN > SportsDB > fallback.
    priority={'official':0,'ncaa':1,'nascar':1,'espn':2,'espn-ncaa':2,'sportsdb':3,'fallback':4,None:5}
    canonical=[];source_counts={};merged=0
    for raw in events:
        candidate=dict(raw);source=candidate.get('source');source_counts[source or 'unknown']=source_counts.get(source or 'unknown',0)+1;match_index=None
        for i,existing in enumerate(canonical):
            if identity_match(existing,candidate):match_index=i;break
        if match_index is None:canonical.append(candidate);continue
        merged+=1;existing=canonical[match_index];ep=priority.get(existing.get('source'),5);cp=priority.get(source,5)
        if cp<ep:
            winner=merge_event_records(candidate,existing);winner['source']=source;canonical[match_index]=winner
        else:canonical[match_index]=merge_event_records(existing,candidate)
