...
# Mobile: Top Sports cards must select their league instead of opening the generic
# source/connect action. The schedule screen then applies an exact league filter.
...
# Mobile host: remember the league selected from Top Sports and open the
# schedule screen already filtered to that league.
text = MAIN.read_text()
text = text.replace(
...
# TV: Top Sports cards navigate to the exact league tab, and every catalog
# sport gets a dedicated branch. Unsupported providers remain empty instead
# of showing another league's games.
text = TV.read_text()
text = text.replace(
    '@Composable fun TvHome(onConnect:()->Unit={},onNetwork:(String)->Unit={}){',
    '@Composable fun TvHome(onConnect:()->Unit={},onNetwork:(String)->Unit={},onSportLeague:(String)->Unit={}){'
)
text = text.replace(
    'TvSportRow(tvSports,onNetwork)',
    'TvSportRow(tvSports){ sport -> onSportLeague(SportsScheduleService.canonicalLeagueFor(sport)) }'
)
# After the schedule patch, TvHome has a consolidated league case. Replace it
# with the complete Top Sports catalog so every tile is isolated.
...
print("Top Sports cards now route to exact league schedules on mobile and TV")
