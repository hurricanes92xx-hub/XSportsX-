...
rows=[]
for path in paths:
    t=time.perf_counter(); data=urllib.request.urlopen(base+path,timeout=10).read()
    rows.append({'path':path,'elapsed_ms':round((time.perf_counter()-t)*1000,2),'bytes':len(data)})
...
[[ -n "$RESOLVED_ACTIVITY" ]] || fail "Could not resolve launcher activity for $PACKAGE (raw: $RESOLVE_RAW)"
...
if [[ "$MODE" == "mobile" ]]; then
  ...
  for attempt in $(seq 1 20); do
    ...
    if ui_has_any "02-live-${attempt}" "LIVE NOW" "No games live right now" "LIVE SPORTS"; then
      log "Live feed ready after ${attempt}s"; cp "$OUT/02-live-${attempt}.png" "$OUT/02-live.png"; cp "$OUT/02-live-${attempt}.xml" "$OUT/02-live.xml"; live_ok=1; break
    fi
  ...
  adb shell input keyevent KEYCODE_HOME; sleep 1; adb shell am start -W -n "$ACTIVITY" >/dev/null; sleep 2
  snapshot 08-relaunch; assert_any_text 08-relaunch "SPORTS COMMAND CENTER" "NEXT-GEN SPORTS COMMAND" "XSPORTS" "ADD SOURCE" "SOURCE READY"
else
  ...
  if has_text "OPEN CONNECTION SETTINGS" 02-tv-settings; then tap_text "OPEN CONNECTION SETTINGS"; sleep 1; fi
  ...
  if ui_has_any 05-after-qr "CONNECT YOUR SOURCE" "SCAN QR CODE"; then log "QR cancellation returned directly to source chooser"; else
  ...
  if has_text "SIGN IN ON TV" 06-source-chooser-return; then tap_text "SIGN IN ON TV"; elif has_text "MANUAL" 06-source-chooser-return; then tap_text "MANUAL"; elif has_text "XTREAM" 06-source-chooser-return; then tap_text "XTREAM"; else tap_text "M3U"; fi
  ...
  if has_text "M3U playlist URL" 10-m3u; then
    ...
    assert_text "TEST & CONNECT" 10-m3u; tap_text "TEST & CONNECT"; sleep 2
    snapshot 11-m3u-result; assert_any_text 11-m3u-result "Connected" "source responded" "SOURCE SAVED" "Connection successful"
  else fail "TV M3U form did not expose playlist URL"; fi
  adb shell input keyevent KEYCODE_HOME; sleep 1; adb shell am start -W -n "$ACTIVITY" >/dev/null; sleep 3
  snapshot 12-tv-home; tap_text "FAVORITES"; sleep 1; snapshot 13-tv-favorites; assert_text "MY TEAMS" 13-tv-favorites; assert_any_text 13-tv-favorites "SELECT YOUR TEAMS" "SELECT MY TEAMS" "BUILD YOUR SPORTS FEED"
