...
def _load():
    try:
        ...
        return value if isinstance(value, dict) else {"schema": 2, "leagues": {}}
    except Exception:
        return {"schema": 2, "leagues": {}}
...
def _score(stat):
    ...
    failures = int(stat.get("consecutiveFailures", 0))
    latency = min(5000.0, float(stat.get("lastLatencyMs", 5000) or 5000))
    events = int(stat.get("lastEventCount", 0) or 0)
    reliability = successes / attempts
    speed = 1.0 - latency / 5000.0
    coverage = 1.0 if events > 0 else 0.0
    return round(max(0.0, reliability * 70.0 + coverage * 20.0 + speed * 10.0 - failures * 15.0), 2)

def record(league, provider, ok, event_count, latency_ms=0, error=""):
    state = _load(); leagues = state.setdefault("leagues", {})
    providers = leagues.setdefault(league, {})
    stat = providers.setdefault(provider, {"attempts": 0, "successes": 0, "consecutiveFailures": 0})
    stat["attempts"] = int(stat.get("attempts", 0)) + 1
    stat["lastChecked"] = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    stat["lastEventCount"] = int(event_count or 0); stat["lastLatencyMs"] = round(float(latency_ms or 0), 1)
    stat["lastError"] = str(error or "")[:300]
    if ok and event_count > 0:
        stat["successes"] = int(stat.get("successes", 0)) + 1; stat["consecutiveFailures"] = 0
    else:
        stat["consecutiveFailures"] = int(stat.get("consecutiveFailures", 0)) + 1
    stat["score"] = _score(stat); state["schema"] = 2
    state["updatedAt"] = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"); _save(state)
    return stat
...
def provider_order(league, configured):
    ...
    for role, provider in enumerate(configured):
        score=float(stats.get(provider, {}).get("score", 50.0)); ranked.append((score, -role, provider))
