...
These adapters run after the general live sweep. They are deliberately
independent so a flaky shadow provider cannot prevent official coverage.
"""
...
def _post_xml(xml,timeout=12):
    ...
    req=urllib.request.Request("https://www.fivb.org/Vis2009/XmlRequest.asmx",data=body,headers={"User-Agent":UA,"Accept":"*/*","Content-Type":"application/x-www-form-urlencoded; charset=utf-8"},method="POST")
...
def _field(el,*names):
    ...
    for k,v in el.attrib.items():
        if k.lower() in wanted and v:return v
    for child in el:
        if child.tag.split("}")[-1].lower() in wanted and child.text:return child.text.strip()
    return ""
...
def _fivb():
    ...
    raw=None
    for req in requests:
        try:raw=_post_xml(req);break
        except Exception as exc:diagnostics["errors"].append(f"list:{type(exc).__name__}")
    if raw is None:
        diagnostics["status"]="unavailable";return result,diagnostics
    try:root=ET.fromstring(raw)
    except Exception as exc:
        diagnostics["status"]="invalid";diagnostics["errors"].append(f"xml:{type(exc).__name__}");return result,diagnostics
    candidates=[]
    for el in root.iter():
        no=_field(el,"No","MatchNo","NoVolleyMatch")
        home=_field(el,"TeamNameA","TeamAName");away=_field(el,"TeamNameB","TeamBName")
        if not no or not home or not away:continue
        candidates.append((no,home,away,el))
    diagnostics["listRecords"]=len(candidates)
    for no,home,away,el in candidates:
...
def _nascar():
    ...
    for host in ("https://feed.nascar.com","https://feedtest.nascar.com"):
        try:
            ...
            if result:break
        except Exception as exc:diagnostics["errors"].append(f"schedule:{type(exc).__name__}")
    if not diagnostics["liveFeed"] and diagnostics["scheduleRecords"]==0:diagnostics["status"]="unavailable"
    return result,diagnostics
...
def main():
    ...
    payload["events"]=events;payload["officialLiveAdapters"]={"checkedAtUtc":checked,"diagnostics":diagnostics,"liveAdded":added,"liveCorroborated":corroborated}
    payload.setdefault("liveSweep",{})["officialAdapters"]=diagnostics
