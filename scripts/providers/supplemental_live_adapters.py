...
ROOT=Path(__file__).resolve().parents[2];FEED=ROOT/'data/schedule_feed.json';UA='XSportsX-SupplementalAdapters/1.3'
...
def _espn_events(sport,league,days):
 ...
 for day in days:
  ...
  for ev in x.get('events') or []:
   comp=(ev.get('competitions') or [{}])[0];stobj=comp.get('status') or {};typ=stobj.get('type') or {};state=str(typ.get('state') or '').lower();live=state=='in';start=iso(ev.get('date'));nm=str(ev.get('name') or ev.get('shortName') or league)
   if start:out.append((row(league,nm,start,f'espn-{league}',f'espn:{ev.get("id")}',live),state))
 return out
def f1():
 ...
 for r,state in _espn_events('racing','f1',days):diag['espn']=True;diag['espnEvents']+=1;out.append(r);diag['live']+=int(r['tag']=='LIVE');diag['upcoming']+=int(r['tag']=='UPCOMING')
 ...
 return out,diag
def nascar():
 now=datetime.now(timezone.utc);out=[];diag={'status':'ok','official':False,'espn':False,'core':False,'espnEvents':0,'live':0,'sessionFallback':0,'errors':[]}
 for v in ('1','2'):
  try:
 ...
 days=[now.date()+timedelta(days=d) for d in (-1,0,1)]
 for r,state in _espn_events('racing','nascar-truck',days):diag['espn']=True;diag['espnEvents']+=1;diag['sessionFallback']+=1;diag['live']+=int(r['tag']=='LIVE');out.append(r)
 # ESPN core events is an independent fallback from the site scoreboard.
 try:
  x=get('https://sports.core.api.espn.com/v2/sports/racing/leagues/nascar-truck/events?limit=1000');diag['core']=True
  for ev in x.get('items') or []:
   ref=ev.get('$ref') or ''
   if not ref:continue
   try:d=get(ref);status=((d.get('competitions') or [{}])[0].get('status') or {}).get('type') or {};state=str(status.get('state') or '').lower();st=iso(d.get('date'));nm=str(d.get('name') or 'NASCAR Craftsman Truck Series');
   except Exception:continue
   if st and datetime.fromisoformat(st.replace('Z','+00:00'))>=now-timedelta(days=1):out.append(row('NASCAR Truck',nm,st,'espn-core-nascar-truck',f"espn-core:{d.get('id')}",state=='in'));diag['live']+=int(state=='in')
...
def _cricket_live_status(status):
 if isinstance(status,dict):
  parts=[str(status.get(k) or '') for k in ('type','state','name','shortDetail','detail','description')]
  typ=status.get('type')
  if isinstance(typ,dict):parts += [str(typ.get(k) or '') for k in ('state','name','shortDetail','detail')]
  return bool(re.search(r'live|in progress|inprogress|stumps|innings|play', ' '.join(parts), re.I)) and not bool(re.search(r'final|postponed|cancelled|scheduled', ' '.join(parts), re.I))
  ...
  for sport in x.get('sports') or []:
   for league in sport.get('leagues') or []:
    ...
    for ev in league.get('events') or []:
     ...
     # The personalized header is discovery; summary is the live-state authority.
     if lid and eid:
      try:
       sm=get(f'https://site.web.api.espn.com/apis/site/v2/sports/cricket/{lid}/summary?contentorigin=espn&event={eid}&lang=en&region=in');diag['espnSummary']+=1;hs=((sm.get('header') or {}).get('competitions') or [{}])[0];live=live or _cricket_live_status(hs.get('status') or sm.get('gameInfo'))
       start=iso(hs.get('date') or start);title=str(hs.get('name') or title)
...
def _walk_json(v,found):
 if isinstance(v,dict):
  if ('startDate' in v or 'start' in v) and ('name' in v or 'title' in v):found.append(v)
...
def xgames():
 ...
 try:
  ...
  blobs=re.findall(r'<script[^>]*id=["\']__NEXT_DATA__["\'][^>]*>(.*?)</script>',html,re.I|re.S)
  found=[]
  for raw in blobs:
   try:_walk_json(json.loads(raw),found)
   except Exception:pass
  # Also parse any JSON-LD Event records when present.
  for raw in re.findall(r'<script[^>]+type=["\']application/ld\+json["\'][^>]*>(.*?)</script>',html,re.I|re.S):
   try:_walk_json(json.loads(raw),found)
   except Exception:pass
  seen=set()
  for obj in found:
   st=iso(obj.get('startDate') or obj.get('start') or obj.get('startTime'));title=str(obj.get('name') or obj.get('title') or '').strip()
   if not st or not title:continue
   key=(norm(title),st)
   if key in seen:continue
   seen.add(key);out.append(row('X Games',title,st,'xgames-official',f'xgames:{norm(title)}:{st}',False));diag['events']+=1
