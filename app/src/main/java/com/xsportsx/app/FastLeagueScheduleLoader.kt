...
/**
 * Fast, bounded league-screen loader. One scoreboard request covers the whole
 * visible horizon; the screen never waits on a multi-request fallback cascade.
 */
    ...
    suspend fun load(league: String, daysAhead: Int = 3): List<SportsEvent> = withContext(Dispatchers.IO) {
        ...
        for (base in bases) {
            val result = withTimeoutOrNull(TIMEOUT_MS) {
                runCatching { parse(JSONObject(get("$base$urlSuffix")), ep, canonical) }.getOrDefault(emptyList())
            }.orEmpty()
