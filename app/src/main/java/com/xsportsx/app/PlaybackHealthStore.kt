...
class PlaybackHealthStore(context: Context) {
    ...
    fun recordSuccess(eventId: String, stream: ResolvedStream, latencyMs: Long = Long.MAX_VALUE) {
        ...
        val key = key(eventId, stream)
        val old = map[key]
        map[key] = Health((old?.successes ?: 0) + 1, old?.failures ?: 0, System.currentTimeMillis(), old?.lastFailureMs ?: 0L, minOf(old?.latencyMs ?: Long.MAX_VALUE, latencyMs))
        trim(); persist()
    }
    ...
    fun recordFailure(eventId: String, stream: ResolvedStream) {
        ...
        val key = key(eventId, stream)
        val old = map[key]
        map[key] = Health(old?.successes ?: 0, (old?.failures ?: 0) + 1, old?.lastSuccessMs ?: 0L, System.currentTimeMillis(), old?.latencyMs ?: Long.MAX_VALUE)
        trim(); persist()
    }

    @Synchronized
    fun score(eventId: String, stream: ResolvedStream): Double {
        val h = map[key(eventId, stream)] ?: return 0.0
    ...
    private fun key(eventId: String, stream: ResolvedStream): String = "$eventId|${stream.url}"
    private fun trim() { while (map.size > 768) map.remove(map.entries.first().key) }
