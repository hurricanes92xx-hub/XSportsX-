...
class DeviceSyncStore(context: Context) {
    private val prefs = context.getSharedPreferences("xsportsx_device", Context.MODE_PRIVATE)
    private fun key(): SecretKeySpec {
        val raw = MessageDigest.getInstance("SHA-256").digest((context.packageName + ":xsportsx-device-v1").toByteArray())
    ...
    private fun load(name: String): String? = runCatching {
        val blob = Base64.decode(prefs.getString(name, null), Base64.NO_WRAP)
        val cipher = Cipher.getInstance("AES/GCM/NoPadding"); cipher.init(Cipher.DECRYPT_MODE, key(), GCMParameterSpec(128, blob, 0, 12))
