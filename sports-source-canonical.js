...
const MLB_ALIASES=new Map([
 ['arizona','diamondbacks'],['arizona diamondbacks','diamondbacks'],['atlanta','braves'],['baltimore','orioles'],['boston','red sox'],['chicago white sox','white sox'],['chicago','cubs'],['cincinnati','reds'],['cleveland','guardians'],['colorado','rockies'],['detroit','tigers'],['houston','astros'],['kansas city','royals'],['los angeles angels','angels'],['la angels','angels'],['los angeles','dodgers'],['miami','marlins'],['milwaukee','brewers'],['minnesota','twins'],['new york mets','mets'],['new york','yankees'],['oakland','athletics'],['philadelphia','phillies'],['pittsburgh','pirates'],['san diego','padres'],['san francisco','giants'],['seattle','mariners'],['st louis','cardinals'],['st. louis','cardinals'],['tampa bay','rays'],['texas','rangers'],['toronto','blue jays'],['washington','nationals']
]);
function canonicalTeam(league,name){const n=norm(name);if(norm(league)==='mlb')return MLB_ALIASES.get(n)||n;return n;}
function eventIdentity(e){const league=norm(e.league||'');const start=Date.parse(e.startUtc||e.start||'');const bucket=Number.isNaN(start)?'unknown':String(Math.floor(start/(2*60*60*1000)));const away=canonicalTeam(e.league,e.away||'');const home=canonicalTeam(e.league,e.home||'');if(away&&home)return `${league}|${away}|${home}|${bucket}`;return `${league}|${norm(e.title||'')}|${bucket}`;}
...
function loadFeed(){try{const raw=JSON.parse(fs.readFileSync(FEED,'utf8'));const sourceRows=Array.isArray(raw.events)?raw.events.map(feedEvent).filter(e=>e.startUtc):[];const unique=new Map();for(const e of sourceRows){const key=eventIdentity(e);const old=unique.get(key);if(!old){unique.set(key,e);continue;}const oldPriority=old.source==='official_api'||old.source==='official'?0:old.source==='espn'||old.source==='ncaa'||old.source==='nascar'?1:2;const newPriority=e.source==='official_api'||e.source==='official'?0:e.source==='espn'||e.source==='ncaa'||e.source==='nascar'?1:2;if(newPriority<oldPriority||(newPriority===oldPriority&&e._index<old._index))unique.set(key,e);}const rows=[...unique.values()];setHealth('canonical-feed',rows.length>0,rows.length,sourceRows.length!==rows.length?`deduped ${sourceRows.length-rows.length} duplicate provider events`:'');return rows.sort((a,b)=>Date.parse(a.startUtc)-Date.parse(b.startUtc));}catch(e){setHealth('canonical-feed',false,0,e.message);return[];}}
...
const VERSION='3.1.0-canonical-dedupe';
