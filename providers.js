...
async function getNcaafRankings() {
  ...
  try {
    const response = await fetch("https://site.api.espn.com/apis/site/v2/sports/football/college-football/rankings?seasontype=2&type=0&level=3", {
    ...
    for (const entry of entries) {
      ...
      const info = {
        ...
        record: entry?.recordSummary || entry?.record || "",
        points: entry?.points ?? null,
        firstPlaceVotes: entry?.firstPlaceVotes ?? entry?.firstPlaceVotesReceived ?? 0,
        previousRank: entry?.previous ?? null,
        movement: entry?.trend || entry?.movement || ""
      };
...
function findRanking(team, rankings) {
  ...
  for (const candidate of candidates) {
    if (rankings.has(candidate)) return rankings.get(candidate);
  }
  for (const candidate of candidates) {
...
function enrichNcaafEvent(event, rankings) {
  ...
  const homeRank = findRanking(event.home, rankings);
  const awayRank = findRanking(event.away, rankings);
  if (!homeRank && !awayRank) return event;
  return {
    ...event,
    home: homeRank ? { ...event.home, ranking: homeRank.rank, rankingInfo: homeRank } : event.home,
    away: awayRank ? { ...event.away, ranking: awayRank.rank, rankingInfo: awayRank } : event.away,
    rankings: [awayRank, homeRank].filter(Boolean).map(x => `#${x.rank} ${x.name}`).join(" vs ")
  };
}
