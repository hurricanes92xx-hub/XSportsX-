...
source=source.replace('if(i>=0&&p[i+1]&&!['manifest.json','configure','health'].includes(p[i+1]))return decryptConfig(decodeURIComponent(p[i+1]));','if(i>=0){for(let j=i+1;j<p.length;j++){if(!["manifest.json","configure","health","catalog","meta","stream"].includes(p[j])){const cfg=decryptConfig(decodeURIComponent(p[j]));if(cfg)return cfg}}}');
...
if(start>=0){const next=source.indexOf("async function ",start+20);if(next>start){const resolver=`async function streamsForEvent(c,meta){
 if(!c||!meta?.event)return[];const key=\`name-epg:\${c.server}|\${c.username}|\${meta.id}\`,cached=matchCache.get(key);if(cached&&Date.now()-cached.at<30000)return cached.value;
 const d=await xtreamData(c),all=Array.isArray(d.metas)?d.metas:[],e=meta.event,rows=[],seen=new Set(),home=[...teamTokens(e.home||{})],away=[...teamTokens(e.away||{})],broadcasts=(e.broadcast||[]).map(norm).filter(Boolean);
 ...
 // Last resort: never return an empty list solely because EPG is missing.
...
fs.writeFileSync(file,source,"utf8");console.log("[XSportsX] universal event-name + EPG-title/description matcher active");
