...
// Keep configuration tokens valid across gateway/backend prefix versions.
...
// Replace the entire stream resolver by locating its function boundaries rather
// than using a fragile regex. This makes the patch idempotent and prevents the
// Render startup crash we saw previously.
...
if (start >= 0) {
  ...
  const all=Array.isArray(d.metas)?d.metas:[];
  ...
  // 1. Search ALL provider channels. A game may be carried on a normal
  // numbered/local/news channel and not in a sports category.
  for(const m of all){
    const text=norm(\`\${m.name||""} \${m.xtream?.category||""}\`);
    let s=0;
  ...
  // 2. Search the provider EPG for every channel concurrently. This is the
  // primary match path because channel names are often generic.
  let cursor=0;
  const hits=[];
  const worker=async()=>{
    while(true){
      ...
      const m=all[n],sid=m.xtream?.streamId;
      if(!sid||seen.has(m.id))continue;
      try{
        const epg=await getEpg(c,sid);
        let best=0,bestE=null;
        for(const e of epg||[]){
          const title=decode(e?.title||""),desc=decode(e?.description||""),txt=norm(\`\${title} \${desc}\`);
          let s=score(m,e,meta);
          for(const t of terms)if(t&&txt.includes(t))s=Math.max(s,t.length>=5?82:55);
          for(const b of broadcasts)if(b&&txt.includes(b))s=Math.max(s,88);
          if(e?.now_playing===1||e?.now_playing==="1")s+=10;
          if(s>best){best=s;bestE=e;}
        }
        if(bestE)hits.push({m,score:best,e:bestE});
      }catch{}
    }
  };
  await Promise.race([
    Promise.all(Array.from({length:Math.min(24,Math.max(1,all.length))},worker)),
    new Promise(r=>setTimeout(r,12000))
  ]);
  for(const h of hits.sort((a,b)=>b.score-a.score).slice(0,32))push(h.m,h.score,h.e);

  // 3. If the provider has no usable EPG, don't return an empty result. Keep
  // likely live channels as candidates so Nuvio can try the authorized source.
  if(rows.length===0){
  ...
  const value=rows.sort((a,b)=>(b.score||0)-(a.score||0)).slice(0,32);
...
fs.writeFileSync(file,"utf8"===undefined?"":source,"utf8");
