...
source = source.replace('set("limit","20")', 'set("limit","20")');
...
async function streamsForEvent(c,meta){
  ...
  // incomplete/mislabelled EPG from producing "No streams found". Results are
  // still ranked by now-playing status, event time, league and team overlap.
  if(rows.length===0){
    const fallback=[];let j=0;
    const fw=async()=>{while(true){const n=j++;if(n>=all.length)return;const m=all[n],sid=m.xtream?.streamId;if(!sid)continue;try{const epg=await getEpg(c,sid);for(const p of epg||[]){let s=10;if(p?.now_playing===1||p?.now_playing==="1")s+=35;const text=norm((p?.title||"")+" "+(p?.description||""));for(const x of eventTerms)if(x&&text.includes(x))s+=x.length>=5?35:20;if(meta.league&&text.includes(norm(meta.league)))s+=12;fallback.push({m,s,p})}}catch{}}};
