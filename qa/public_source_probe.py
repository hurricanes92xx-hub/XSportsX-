#!/usr/bin/env python3
"""End-to-end probe for every enabled public playlist in public-sources-registry.json."""
...
def fetch(url, headers=None):
    ...
    with urlopen(Request(url,headers=h),timeout=TIMEOUT) as r:
        return r.status,r.headers.get('content-type',''),r.read(),time.time()-t

def parse(data):
    lines=[x.strip() for x in data.decode('utf-8','replace').splitlines()]
    out=[]; meta=None
    for x in lines:
    ...
    for s in sources:
        try:
            ...
            for _,_,url in samples:
                try:
                    r,_,_,_=fetch(url,{'Range':'bytes=0-1023'})
                    if 200 <= r < 500: reachable+=1
                except Exception: pass
            result={'id':s['id'],'name':s['name'],'playlist_status':status,'entries':len(entries),'sports_entries':len(sports),'sample_streams':len(samples),'reachable_streams':reachable,'elapsed_sec':round(elapsed,2)}
            print(json.dumps(result,separators=(',',':')))
            if status >= 400 or not entries: failed+=1
        except Exception as e:
            failed+=1; print(json.dumps({'id':s['id'],'name':s['name'],'error':str(e)[:240]}))
    print(f'PUBLIC_SOURCE_QA_RESULT failures={failed} total={len(sources)}')
